...
-  already used by option '${matchingOption.flags}'`);
      ...
      _prepareUserArgs(argv, parseOptions) {
        ...
        if (argv === void 0 && parseOptions.from === void 0) {
          ...
          const execArgv = process15.execArgv ?? [];
...
var storyCmd = new Command("story").description(
  "User story management"
);
storyCmd.command("list").description(
  "List user stories"
).action(async () => {
  const stories = await listStories();
  console.log(stories);
});
storyCmd.command("set").requiredOption(
  "--id <id>",
  "Story ID"
).description(
...
storyCmd.command("show").description(
  "Show current story"
).action(async () => {
  const config = getConfig();
...
// node_modules/restore-cursor/index.js
var terminal = import_node_process8.default.stderr.isTTY ? import_node_process8.default.stderr : import_node_process8.default.stdout.isTTY ? import_node_process8.default.stdout : void 0;
...
var isHidden = false;
var cliCursor = {};
cliCursor.show = (writableStream = import_node_process9.default.stderr) => {
...
cliCursor.hide = (writableStream = import_node_process9.default.stderr) => {
  if (!writableStream.isTTY) {
    return;
  }
...
function isUnicodeSupported() {
  const { env: env3 } = import_node_process10.default;
...
var StdinDiscarder = class {
  ...
  #handleInputBound = (chunk) => {
    ...
    if (code === ASCII_ETX_CODE) {
      import_node_process11.default.kill(import_node_process11.default.pid, "SIGINT");
    }
  };
  start() {
    this.#activeCount++;
  ...
  #realStart() {
    const { stdin } = import_node_process11.default;
    if (import_node_process11.default.platform === "win32" || !stdin?.isTTY || typeof stdin.setRawMode !== "function") {
      this.#stdin = void 0;
      return;
    }
    this.#stdin = stdin;
...
var Ora = class {
  ...
  constructor(options) {
    ...
    this.#options = {
      color: "cyan",
      stream: import_node_process12.default.stderr,
      discardStdin: true,
      hideCursor: true,
      ...options
    };
    ...
    if (import_node_process12.default.env.NODE_ENV === "test") {
  ...
  */
  #installHook() {
    ...
    const streamsToHook = /* @__PURE__ */ new Set([this.#stream, import_node_process12.default.stdout, import_node_process12.default.stderr]);
  ...
  start(text) {
    ...
    if (this.#options.discardStdin && import_node_process12.default.stdin.isTTY) {
...
// src/utils/poller.ts
async function pollUntilDone(fetchFn) {
  ...
  while (true) {
    ...
    spinner.text = `Current Status: ${status}`;
    if (status === "completed") {
      spinner.succeed(
        `Completed: ${status}`
      );
...
var deployCmd = new Command("deploy").description(
  ...
  console.log(
    source_default.yellow(
      `\u{1F680} Deploying to ${opts.env}`
    )
  );
  console.log(
    source_default.blue(
      "\u{1F4E6} Packaging metadata..."
    )
  );
  await new Promise(
    (r) => setTimeout(r, 1500)
...
var commitCmd = new Command("commit").requiredOption(
  "-s, --story <id>"
).action(async (opts) => {
  const result = await commit(opts.story);
  console.log(result);
});

// src/commands/promote.ts
var promoteCmd = new Command("promote").requiredOption(
  "-s, --story <id>"
).action(async (opts) => {
  const result = await promote(opts.story);
  console.log(result);
});

// src/commands/validate.ts
var validateCmd = new Command("validate").requiredOption(
  "-s, --story <id>",
  "User Story ID"
).option(
  "-w, --watch",
...
var envsCmd = new Command("envs").description("List environments").action(async () => {
  const envs = await listEnvironments();
  console.log(envs);
});

// src/commands/jobs.ts
var jobsCmd = new Command("jobs");
jobsCmd.command("poll").requiredOption(
  "-i, --id <id>"
).action(async (opts) => {
  const result = await pollJob(opts.id);
  console.log(result);
});
...
var testCmd = new Command("test").description(
  "CRT testing commands"
);
testCmd.command("jobs").description(
  "List test jobs"
).action(async () => {
  const jobs = await listJobs();
  console.log(jobs);
});
testCmd.command("run").requiredOption(
  "--job <id>",
  "Test Job ID"
).option(
  "-w, --watch",
  ...
  console.log(
    `\u{1F9EA} Running test job ${opts.job}`
  );
  if (opts.watch) {
    await pollUntilDone(
      () => pollBuild(opts.job)
...
// src/api/ai.ts
var askAgent = async (agent, message) => {
  return {
    agent,
    message,
    response: `AI Agent "${agent}" processed: ${message}`
  };
};

// src/commands/ai.ts
var aiCmd = new Command("ai").description(
  "AI Agent commands"
);
aiCmd.command("ask").requiredOption(
  "--agent <agent>",
  "Agent name"
).argument(
  "<message>"
...
var pipelineCmd = new Command("pipeline").description(
  "Run complete DevOps pipeline"
);
pipelineCmd.command("run").description(
  "Execute full pipeline"
).action(async () => {
  console.log(
    source_default.green(
      "\n\u{1F680} Starting Copado Pipeline\n"
    )
  );
  console.time(
    source_default.cyan(
  ...
  console.log(
    source_default.blue(
      "\u{1F4E6} Packaging deployment artifacts..."
    )
  );
  await new Promise(
    (r) => setTimeout(r, 1500)
  ...
  console.log(
    source_default.magenta(
      "\u{1F916} AI Release Summary Generated"
    )
  );
  console.log(
...
program2.name("copado-hx").description(
  "Headless CLI for Copado"
).version("1.0.0");
program2.addCommand(authCmd);
program2.addCommand(storyCmd);
program2.addCommand(deployCmd);
program2.addCommand(commitCmd);
program2.addCommand(promoteCmd);
program2.addCommand(validateCmd);
program2.addCommand(envsCmd);
program2.addCommand(jobsCmd);
program2.addCommand(testCmd);
program2.addCommand(aiCmd);
program2.addCommand(pipelineCmd);
program2.parse();
